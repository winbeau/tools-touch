...
实现 Gmail 首次登录门槛、登录邮箱兼作投递邮箱、加密恢复登录；后台自动加载组件、进度提示；服务商与模型配置、浏览器/设备码/API Key、授权取消及忙碌状态修复；Google 配置导入/预置、回调可靠性和诊断日志。最终原生测试 12 组通过；真实 Google 客户端配置及真实账户验收仍未完成，详见 ACCEPTANCE.md 最新记录。
