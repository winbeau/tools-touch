...
0.3 系列请使用 [v0.3.3 安装器](https://github.com/winbeau/tools-touch/releases/download/v0.3.3/ToolsTouch-Setup-0.3.3-win-x64.exe)。此版统一 11 页浅色工作台、左侧导航、邮件与表格交互，保留随包 Google 桌面登录配置；Google 项目审核与真实账号验收状态仍见发布说明。
...
```powershell
python scripts/build-installer.py --package artifacts/ToolsTouch-win-x64-v0.3.3.zip --version 0.3.2 --output artifacts/release-v0.3.3
python scripts/test-installer.py --installer artifacts/release-v0.3.3/ToolsTouch-Setup-0.3.3-win-x64.exe --test-exe artifacts/desktop-tests-v0.3.3/ToolsTouch.Desktop.Tests.exe --output artifacts/installer-check
```
...
构建器还会检查完整的四层应用程序集和 Node／Python 生产依赖。可为 `test-installer.py` 增加 `--previous-installer <旧版安装器.exe>`，先在同一临时目录安装旧版，再执行升级与用户文件保留检查。检查同时拒绝当前用户／整机以及 32／64 位注册表视图中的既有安装。
...
已增加 Windows 11 原生自动检查，覆盖 11 个页面与详情子页渲染、界面绑定、草稿编辑、DPAPI 跨进程读取和 Pi 连接后退出。测试用合成资料和独立临时目录；真实账号、人工完整操作及干净 Windows 用户验收仍需完成。不要将自动检查当作完整验收。
...
可用 `uv run --all-packages --locked python scripts/verify-package.py artifacts/ToolsTouch-win-x64-v0.3.3` 验证生成目录的全部文件哈希。校验只证明产物与构建清单一致，不是代码签名或 Windows 运行验证。
...
```powershell
python scripts/test-windows.py --package artifacts/ToolsTouch-win-x64-v0.3.3 --output artifacts/windows-check
```
...
如果使用 WSL 交叉构建，可先执行以下命令，再从 Windows 调用上面的脚本并追加 `--test-exe artifacts/desktop-tests-v0.3.3/ToolsTouch.Desktop.Tests.exe`，Windows 侧无需另外安装 SDK：

```sh
dotnet publish tests/ToolsTouch.Desktop.Tests -c Release -r win-x64 --self-contained true -o artifacts/desktop-tests-v0.3.3
```
...
- 干净 Windows 用户解压运行；确认 11 个页面渲染、选择、滚动、命令可用状态和错误提示正常。
